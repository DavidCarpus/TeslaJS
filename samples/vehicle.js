...
function sampleMain(tjs, options) {
  tjs.vehicleDataAsync(options).then(function (vehicleData) {
    ...
    console.log(
      "Battery level: " +
        charge_state.battery_level.toString().green +
        " / ".green +
        charge_state.charge_limit_soc.toString().green +
        " %".green
    );
    ...
    console.log(
      "\nRated range: " +
        (unitsInKms
          ? Math.round(milesToKms(charge_state.battery_range)).toString()
              .green + " km"
          : Math.round(charge_state.battery_range).toString().green + " mi")
    );
    console.log(
      (eu_vehicle ? "Typical" : "Ideal") +
        " range: " +
        (unitsInKms
          ? Math.round(milesToKms(charge_state.ideal_battery_range)).toString()
              .green + " km"
          : Math.round(charge_state.ideal_battery_range).toString().green +
            " mi")
    );
    console.log(
      "Projected range: " +
        (unitsInKms
          ? Math.round(milesToKms(charge_state.est_battery_range)).toString()
              .green + " km"
          : Math.round(charge_state.est_battery_range).toString().green + " mi")
    );
    ...
    console.log(
      "\nIs user present? ",
      vehicle_state.is_user_present ? "YES".yellow : "NO".green
    );
