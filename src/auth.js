  ...
  req({
    ...
    .then(function (result) {
      ...
      return req({
        method: "POST",
        url:
          (url.query.issuer || "https://auth.tesla.com/oauth2/v3") + "/token",
        jar: false,
        json: true,
        headers: {
          Accept: "*/*",
...
function mfaVerify(transactionId, host, referer, mfaPassCode, mfaDeviceName) {
  return req({
    method: "GET",
    url:
      host + "/oauth2/v3/authorize/mfa/factors?transaction_id=" + transactionId,
    headers: {
    ...
    .then(function (result) {
      ...
      if (mfaDeviceName) {
        ...
        device = result.body.data.find(function (dev) {
          return dev.name == mfaDeviceName;
        });
        if (!device) {
